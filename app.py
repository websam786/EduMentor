...
social = st.number_input("Social studies Marks" , min_value=0, max_value=100)
...
if st.button("Analyze Performance"):
    ...
    response = requests.post(
        "http://127.0.0.1:8000/analyze-student",
        json=student_data
    ...
    with col2:
        st.metric("Overall", f'{result["percentage"]:.2f}%')
    ...
    feedback_card("Summary", "📋", feedback["summary"], "#2563EB")
    feedback_card("Strengths", "💪", feedback["strength"], "#16A34A")
    feedback_card("Areas to Improve", "📚", feedback["areas_to_improve"], "#EA580C")
    feedback_card("Tomorrow's Study Plan", "📅", feedback["study_plan"], "#7C3AED")

else:
    st.error("Something went wrong while analyzing the student.")
